...
from app.repositories.course_repo import EnrollmentRepository, CourseRepository
from app.repositories.attendance_repo import AttendanceAttemptRepository, FinalAttendanceRepository
from app.services.face_service import FaceService
from app.services.audit_service import log_action
from app.utils.location import verify_location
...
class AttendancePipelineService:
    ...
    def scan_qr(self, student: User, session_id: int, qr_token: str) -> AttendanceAttempt:
        session = self.session_repo.get_by_id(session_id)
        if not session or session.status != "active":
            raise HTTPException(status_code=404, detail="Aktif oturum bulunamadı")

        if session.qr_token != qr_token:
            raise HTTPException(status_code=400, detail="QR kod geçersiz veya süresi dolmuş")
        ...
        if session.qr_token_issued_at is not None:
            ...
            if age_seconds > qr_ttl:
                raise HTTPException(status_code=400, detail="QR kodunun süresi dolmuş. Lütfen yeni kodu taratın.")
        ...
        if existing_final:
            raise HTTPException(status_code=409, detail="Bu oturum için zaten yoklama işaretlediniz")
    ...
    def verify_face(self, student: User, session_id: int,
                    image_base64: str, image_base64_2: Optional[str] = None) -> AttendanceAttempt:
        attempt = self.attempt_repo.get_by_student_session(student.id, session_id)
        ...
        if not self.face_service.engine.is_available:
            attempt = self.attempt_repo.update(attempt, face_status="verified", face_confidence=0.0)
            return attempt
        ...
        attempt = self.attempt_repo.update(attempt, face_status=status, face_confidence=confidence)
    ...
    def verify_location(self, student: User, session_id: int,
        ...
        attempt = self.attempt_repo.update(attempt, completed_at=datetime.now(timezone.utc))
        ...
        log_action(
            self.db, "attendance_marked",
            actor_id=student.id, actor_role="student",
            resource="final_attendance_record", resource_id=final_record.id,
            detail={"session_id": session_id, "status": record_status, "is_flagged": is_flagged, "flag_reason": flag_reason},
        )
        ...
        return {
            "success": True,
            "message": "Yoklama başarıyla kaydedildi" if not is_flagged else "Yoklama kaydedildi, öğretmen incelemesi gerekiyor",
    ...
    def manual_attendance(self, session_id: int, student_id: int,
        ...
        if existing:
            raise HTTPException(status_code=409, detail="Bu öğrenci için yoklama zaten kaydedildi")
        ...
        record = self.final_repo.create(
            ...
            is_flagged=is_flagged,
            flag_reason=flag_reason,
            verification_steps={"manual": True, "face_used": face_used, "face_confidence": confidence},
            status=record_status,
        )
        return {
            "success": True,
    ...
    def web_attend(self, student: User, session_id: int,
                   image_base64: str, latitude: float, longitude: float,
        ...
        if not location_ok and not location_skipped:
            reasons.append(f"location_failed ({distance_m:.0f}m)" if distance_m else "location_failed")
        ...
        if is_flagged:
            self._notify_instructor_flagged(session, student, flag_reason or "web_verification_failed")

        return {
            "success": True,
            "message": "Yoklama kaydedildi" if not is_flagged else "Yoklama kaydedildi, öğretmen incelemesi gerekiyor",
            "is_flagged": is_flagged,
            "flag_reason": flag_reason,
            "face_ok": face_ok,
            "location_ok": location_ok or location_skipped,
